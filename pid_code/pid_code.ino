...
const int maxSpeedR = 64;
const int baseSpeedL = 65;
const int baseSpeedR = 30;

// PID variables
double input, output, setpoint;
double Kp = 0.06, Ki = 0.0, Kd = 2.0;  // Tune these!
PID pid(&input, &output, &setpoint, Kp, Ki, Kd, DIRECT);

// Output scaling
const float outMultL = 1.0;
const float outMultR = float(baseSpeedL) / baseSpeedR; // ~2.16, tune if needed
...
void loop() {
  ...
  int leftSpeed  = baseSpeedL + output * outMultL;
  int rightSpeed = baseSpeedR - output * outMultR;
